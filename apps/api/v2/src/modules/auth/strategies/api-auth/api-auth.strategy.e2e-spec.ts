...
describe("ApiAuthStrategy", () => {
  ...
  beforeAll(async () => {
    ...
    const data = {
      logo: "logo-url",
      name: "name",
      redirectUris: ["https://cal.log1.com"],
      permissions: 32,
    };
  ...
  describe("authenticate with strategy", () => {
      ...
      const context: ExecutionContext = {
        switchToHttp: () => ({
          getRequest: () => ({
            ...
            get: (key: string) =>
              ({ Authorization: `Bearer cal_test_badkey1234`, origin: "https://cal.log1.com" }[key]),
          }),
        }),
      } as ExecutionContext;
    ...
    it("should throw 401 if request does not contain Bearer token nor OAuth client credentials", async () => {
      const context: ExecutionContext = {
        switchToHttp: () => ({
          getRequest: () => ({
            get: (key: string) => ({ Authorization: ``, origin: "https://cal.log1.com" }[key]),
          }),
        }),
      } as ExecutionContext;
      const request = context.switchToHttp().getRequest();
